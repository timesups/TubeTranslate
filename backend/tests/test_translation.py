...
def test_concurrency_from_bad_saved_values_falls_back_to_default(value):
    assert openai_translate._concurrency_from({"translate_concurrency": value}) == 8


def test_concurrency_from_caps_effective_workers():
    assert openai_translate._concurrency_from({"translate_concurrency": "50"}) == 16
    assert openai_translate._concurrency_from({"translate_concurrency": "8"}) == 8
    assert openai_translate._concurrency_from({"translate_concurrency": "1"}) == 1
...
@pytest.mark.parametrize(
    ...
    [
        ('{"dst": "两个，"}', "两个，"),
        ('{"dst": "两个，"', "两个，"),  # truncated closing brace
        ('```json\n{"dst": "你好"}\n```', "你好"),
        ('Here you go:\n{"dst": "ok"}\n', "ok"),
        ('{"dst": "he said \\"hi\\""}', 'he said "hi"'),
    ],
)
def test_extract_json_recovers_common_model_outputs(raw, expected):
    assert openai_translate._extract_json(raw) == {"dst": expected}
...
def test_translate_batch_reports_progress(monkeypatch):
    monkeypatch.setattr(openai_translate, "_call_json", lambda *a, **kw: {"dst": "x"})
    monkeypatch.setattr(openai_translate, "_client", lambda *a, **kw: object())
    ...
    assert out == ["x", "x", "x"]
    assert events[0] == (0, "Translating 0/3 sentences")
    assert events[-1][0] == 100
    assert "3/3" in events[-1][1]
