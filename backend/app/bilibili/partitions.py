...
PARTITION_TIDS = frozenset(int(item["tid"]) for item in PARTITIONS)


def partition_name(tid: int) -> str:
    for item in PARTITIONS:
        if int(item["tid"]) == int(tid):
...
def normalize_bilibili_tid(value: int | str | None) -> int:
    if value is None or value == "":
        return DEFAULT_BILIBILI_TID
    tid = int(value)
    if tid not in PARTITION_TIDS:
        raise ValueError(f"Unsupported bilibili_tid: {tid}")
    return tid
