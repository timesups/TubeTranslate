...
def init_db() -> None:
    ...
    with connect() as conn:
        ...
        if "audio_mode" not in task_columns:
            conn.execute(
                "ALTER TABLE tasks ADD COLUMN audio_mode TEXT NOT NULL DEFAULT 'keep_bgm'"
            )
        if "tts_provider" not in task_columns:
            conn.execute(
...
def reclaim_interrupted_tasks() -> list[str]:
    """Re-queue tasks left active (or auto-failed) after a backend process restart."""
    restart_message = "Backend restarted before the task completed."
    with connect() as conn:
        rows = conn.execute(
            """
            SELECT id
            ...
            WHERE status IN ({active})
               OR (status = 'failed' AND error_message = ?)
            ORDER BY rowid ASC
            """.format(active=", ".join("?" for _ in ACTIVE_STATUSES)),
            [*ACTIVE_STATUSES, restart_message],
        ).fetchall()
...
def normalize_audio_mode(value: str | None) -> str:
    mode = (value or DEFAULT_AUDIO_MODE).strip().lower()
    if mode not in AUDIO_MODES:
        raise ValueError(f"audio_mode must be one of: {', '.join(AUDIO_MODES)}")
    return mode
...
def normalize_bilibili_auto_publish(value: bool | int | str | None) -> bool:
    if value is None or value == "":
        return DEFAULT_BILIBILI_AUTO_PUBLISH
    if isinstance(value, bool):
        return value
    if isinstance(value, int):
        if value in (0, 1):
            return bool(value)
        raise ValueError("bilibili_auto_publish must be 0 or 1")
    text = str(value).strip().lower()
    if text in {"1", "true", "yes", "on", "publish"}:
        return True
    if text in {"0", "false", "no", "off", "export"}:
        return False
...
def create_task(
    ...
    *,
    ...
    with connect() as conn:
        ...
        # Keep writing zeros into legacy Douyin columns when present so older DBs stay valid.
        if "douyin_auto_publish" in columns and "douyin_generate_meta" in columns:
            conn.execute(
                """
                INSERT INTO tasks (
...
def list_tasks(limit: int = 100) -> list[dict[str, Any]]:
    ...
    return [_serialize_task_fields(dict(row)) for row in rows]
...
def list_tasks_page(
    *,
    ...
    return {
        "tasks": [_serialize_task_fields(dict(row)) for row in rows],
...
def get_task(task_id: str) -> dict[str, Any] | None:
    ...
    result = _serialize_task_fields(dict(task))
...
def save_azure_tts_settings(
    *,
    ...
    updated_at = now_iso()
    resolved_voice = voice.strip() or defaults["voice"]
    resolved_locale = locale.strip() or defaults["locale"]
    if not locale.strip() and resolved_voice:
        parts = resolved_voice.split("-")
        if len(parts) >= 2 and 2 <= len(parts[0]) <= 3 and 2 <= len(parts[1]) <= 3:
            resolved_locale = f"{parts[0]}-{parts[1]}"

    updates = {
        "azure_tts.region": region.strip() or defaults["region"],
        "azure_tts.voice": resolved_voice,
        "azure_tts.locale": resolved_locale,
        "azure_tts.endpoint": endpoint.strip(),
        "azure_tts.output_format": (
            output_format.strip() or defaults["output_format"]
        ),
        "azure_tts.speech_rate": (
            speech_rate.strip() if speech_rate.strip() != "" else defaults["speech_rate"]
        ),
        "azure_tts.concurrency": (
