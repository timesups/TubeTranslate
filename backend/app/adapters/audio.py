...
# ratio < 1 shortens (speeds up). Keep post-processing within a natural range:
# short clips retain their synthesized cadence, while overflowing clips can be
# accelerated by at most ~33%. More extreme fitting tends to sound erratic.
...
def _fit_ratio(current_sec: float, available_sec: float) -> float:
    """Choose a natural stretch ratio without slowing speech to fill silence.

    A ratio below one is used only when a clip would collide with the next cue,
    and acceleration is capped to avoid abrupt cadence changes.
    """
    if current_sec <= 1e-6:
        return 1.0
    if current_sec <= available_sec + 1e-3:
        return 1.0
    desired_sec = max(MIN_SLOT_SEC, available_sec)
    return max(MIN_STRETCH_RATIO, desired_sec / current_sec)
...
def merge_tts_audio(translation_file: Path, tts_dir: Path, session: Path) -> tuple[Path, Path]:
    ...
    for index, (segment, tts_file) in enumerate(zip(translation, tts_files)):
        ...
        if available_sec < MIN_SLOT_SEC:
            ...
            ratio = _fit_ratio(current_sec, available_sec)
            target_sec = current_sec * ratio
    ...
    if truncated:
        log.warning(
            "Hard-truncated %d severely overflowing TTS segments after applying the "
            "natural-speed limit; consider shortening their translations",
            truncated,
        )
