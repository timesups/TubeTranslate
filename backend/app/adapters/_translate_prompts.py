...
3) 短碎片禁扩写。若原文只有 1-3 个英文词（如 And. / Also. / Like so. / Apply.），译文必须同样短（通常不超过 8 个汉字）；严禁脑补教程开场白、欢迎语或上下文。
4) 一致性与保留项。人名、地名、品牌、型号、库/框架/算法名、缩写（GPU、API、Transformer 等）默认保留原文大小写；广为接受的中文译法须使用，如 LEGO -> 乐高；首次出现的专名可写「中文（原文）」或保留原文，后续保持一致；文件名、函数名、类名、命令、路径、URL、邮箱、哈希、版本号一律保留原样；subscribe the channel 译为「关注」而非「订阅」；AI Agent 译为「AI 智能体」；非常短的语气词（aha、wow、oh、ah、um、uh）保留原文。
5) 纠错。明显错误直接修正后再翻译，不解释、不标注。
6) 数字与单位。数字不加英文千分位逗号（写 6000，不写 6,000）；超大数字（10^8 及以上）改写为「亿/百万」等中文计数；百分数、比值、温度、货币、尺寸保持原单位与格式（3.5%、$12.99、1080p、5 km），不做单位换算；序号保持格式：Section 3 -> 第3节，Figure 2 -> 图2，Table 5 -> 表5。
7) 标点与排版。使用中文标点（，。！？；：「」（））；破折号「——」**禁用**，改用括号或逗号分句；省略号用「…」；引号统一「」或「""」；长句用逗号细分；必须使用标点。
8) 简洁易读。避免生僻词；能口语则不堆砌书面语；语序优先自然中文。
9) 数学符号：α、β、∠、[a, b] 保留符号；alpha plus beta equals angle ABC -> α + β = ∠ABC；公式写成 5 minus 2 -> 5-2、10 times 3 -> 10*3。
10) 代码与命令。`反引号`内容保留原样；命令行、参数、JSON/YAML 键名不译。
11) 表述强度。粗口保留力度（妈的 / 卧槽 / 我去 / 操 / 他妈的，按语境选用）；美式 so 常作语气词「嗯啊哦」，需按语境判断不要僵硬译为「所以」。
