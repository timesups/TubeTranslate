...
def _task_audio_mode(task: dict | None) -> str:
    if not task:
        return database.DEFAULT_AUDIO_MODE
    return task.get("audio_mode") or database.DEFAULT_AUDIO_MODE


def _replace_original_audio(task: dict | None) -> bool:
    return _task_audio_mode(task) == "replace"
...
class PipelineRunner:
    ...
    def _restore_cached_stage(self, stage: str, task: dict | None) -> None:
        ...
        if stage == "separate":
            ...
            if _replace_original_audio(task):
                self.artifacts.bgm_file = bgm_path if bgm_path.exists() else None
            else:
                self.artifacts.bgm_file = _require_existing(bgm_path, "bgm_file")
            return
        if stage == "asr":
    ...
    def _separate(self, task: dict) -> None:
        ...
        if _replace_original_audio(task):
            ...
            self.artifacts.vocals_file = extract_source_audio(video_file, session)
            self.artifacts.bgm_file = None
            self.stage_message(
                "separate",
                f"Skipped Demucs; extracted original audio -> {self.artifacts.vocals_file.name}",
            )
            return
        ...
        self.stage_message(
            "separate",
            f"Vocals: {self.artifacts.vocals_file.name}, BGM: {self.artifacts.bgm_file.name}",
        )
    ...
    def _translate(self, task: dict) -> None:
        ...
        from .adapters.openai_translate import _concurrency_from, translate_asr
    ...
    def _tts(self, task: dict) -> None:
        ...
        if provider == "azure":
            ...
            self.artifacts.tts_dir = generate_azure_tts(
                translation_file,
                session,
                progress_callback=lambda progress, message: self.stage_progress(
                    "tts", progress, message
            ...
            self.stage_message(
                "tts",
                f"Generated {wav_count} Azure TTS clips -> {self.artifacts.tts_dir}",
            )
            return
    ...
    def _merge_video(self, task: dict) -> None:
        ...
        replace_audio = _replace_original_audio(task)
        bgm_file = None if replace_audio else _require(self.artifacts.bgm_file, "bgm_file")
        ...
        mode_note = "replace original audio" if replace_audio else "mix dubbing with BGM"
        self.stage_message(
            "merge_video",
