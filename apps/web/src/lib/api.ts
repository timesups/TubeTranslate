...
export function deleteTasksBatch(taskIds: string[]) {
  return request<BatchDeleteResult>("/api/tasks/batch-delete", {
    method: "POST",
    body: JSON.stringify({ task_ids: taskIds }),
  })
}

export function resumeTasksBatch(taskIds: string[]) {
  return request<BatchResumeResult>("/api/tasks/batch-resume", {
    method: "POST",
    body: JSON.stringify({ task_ids: taskIds }),
  })
}
...
export type TaskBatchResult = {
  created: TaskBatchItem[]
  existing: TaskBatchItem[]
...
export type LocalUploadBatchError = {
  filename: string
  detail: string
}

export type LocalUploadBatchResult = {
  created: Task[]
  errors: LocalUploadBatchError[]
}

export async function uploadLocalTasks(
  files: File[],
  direction: LocalDirection,
  subtitleFile: File | null = null,
  executionMode: ExecutionMode = "auto",
  ...
  const allowSubtitle = files.length === 1
  ...
  for (const file of files) {
    try {
      const task = await uploadLocalTask(
        file,
        direction,
        allowSubtitle ? subtitleFile : null,
        executionMode,
        audioMode,
        ttsProvider,
        bilibiliTid,
...
export function saveAzureTtsSettings(settings: {
  subscription_key: string
  clear_subscription_key?: boolean
  region: string
  voice: string
  locale: string
  endpoint: string
...
export function getAzureTtsVoices(settings: {
  region: string
  subscription_key: string
  endpoint: string
}) {
