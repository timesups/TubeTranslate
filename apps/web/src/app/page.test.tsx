...
describe("本地视频字幕选择", () => {
  ...
  it("可一次选择多个本地视频并分别创建任务", async () => {
    let uploadCount = 0
    mocks.fetch.mockImplementation(async (input: RequestInfo | URL) => {
      const path = String(input)
      if (path === "/api/tasks/upload") {
        uploadCount += 1
        return new Response(JSON.stringify({ id: `task-${uploadCount}` }), {
          status: 200,
          headers: { "Content-Type": "application/json" },
        })
      }
    ...
    await user.upload(videoInput, [videoA, videoB])

    expect(screen.getByTestId("local-upload-selection")).toHaveTextContent("已选择视频: 2")
    expect(screen.getByTestId("local-upload-selection")).toHaveTextContent("video-a.mp4")
    expect(screen.getByTestId("local-upload-selection")).toHaveTextContent("video-b.mp4")
    expect(subtitleInput).toBeDisabled()

    await user.click(screen.getByRole("button", { name: "批量创建" }))

    await waitFor(() => {
      expect(screen.getByText("已上传 2 个本地视频任务，失败 0 个。")).toBeInTheDocument()
    })

    const uploadCalls = mocks.fetch.mock.calls.filter(([input]) => String(input) === "/api/tasks/upload")
    expect(uploadCalls).toHaveLength(2)
    expect((uploadCalls[0][1]?.body as FormData).get("file")).toMatchObject({ name: "video-a.mp4" })
    expect((uploadCalls[1][1]?.body as FormData).get("file")).toMatchObject({ name: "video-b.mp4" })
  ...
  it("支持批量创建多个链接并留在首页展示结果", async () => {
    mocks.fetch.mockImplementation(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.pathname === "/api/tasks/batch" && method === "POST") {
        return new Response(JSON.stringify({
          created: [
            { url: "https://www.youtube.com/watch?v=batchaaa001", task: { id: "batchaaa001" } },
            { url: "https://www.youtube.com/watch?v=batchbbb002", task: { id: "batchbbb002" } },
          ],
          existing: [],
          errors: [],
        }), {
    ...
    await user.click(screen.getByRole("button", { name: "批量创建" }))
    expect(await screen.findByText("新建 2 个，已存在 0 个，失败 0 个。")).toBeInTheDocument()
    expect(mocks.push).not.toHaveBeenCalled()

    const batchCall = mocks.fetch.mock.calls.find(
      ([input, init]) => String(input).includes("/api/tasks/batch") && init?.method === "POST",
    )
...
describe("任务批量删除", () => {
  it("勾选任务后可批量删除并刷新列表", async () => {
    ...
    mocks.fetch.mockImplementation(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (path === "/api/tasks/batch-delete" && method === "POST") {
        const body = JSON.parse(String(init?.body)) as { task_ids: string[] }
        const deleted = body.task_ids.filter((id) => id !== "run-1")
        tasks = tasks.filter((task) => !deleted.includes(task.id))
        return new Response(JSON.stringify({
          deleted,
          skipped: body.task_ids.includes("run-1") ? [{ id: "run-1", reason: "running" }] : [],
          missing: [],
          failed: [],
        }), {
          status: 200,
    ...
    await user.click(screen.getByRole("button", { name: "选中本页已结束" }))
    expect(screen.getByText("已选 2 个")).toBeInTheDocument()
    expect(screen.getByLabelText("选择任务: 运行中任务")).toBeDisabled()

    await user.click(screen.getByRole("button", { name: "删除所选 (2)" }))
    expect(screen.getByText("确认删除所选任务？")).toBeInTheDocument()
    await user.click(screen.getByRole("button", { name: "确认删除" }))

    expect(await screen.findByText("已删除 2 个，跳过运行中 0 个，不存在 0 个，失败 0 个。")).toBeInTheDocument()
    await waitFor(() => {
      expect(screen.queryByText("已完成任务一")).not.toBeInTheDocument()
      expect(screen.queryByText("失败任务一")).not.toBeInTheDocument()
    })
    expect(screen.getByText("运行中任务")).toBeInTheDocument()

    const deleteCall = mocks.fetch.mock.calls.find(
      ([input, init]) => String(input) === "/api/tasks/batch-delete" && init?.method === "POST",
    )
    expect(JSON.parse(String(deleteCall?.[1]?.body))).toEqual({
      task_ids: ["done-1", "fail-1"],
    })
  })
})

describe("任务批量重试", () => {
  it("只对所选失败任务调用 batch-resume", async () => {
    window.localStorage.setItem("youdub-ui-language", "zh")
    let tasks = [
      {
        id: "done-1",
        url: "https://example.com/done-1",
    ...
    mocks.fetch.mockImplementation(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (path === "/api/tasks/batch-resume" && method === "POST") {
        const body = JSON.parse(String(init?.body)) as { task_ids: string[] }
        const resumed = body.task_ids.filter((id) => id.startsWith("fail-"))
        tasks = tasks.map((task) => (
          resumed.includes(task.id) ? { ...task, status: "queued", error_message: null } : task
        ))
        return new Response(JSON.stringify({
          resumed,
          skipped: body.task_ids
            .filter((id) => !id.startsWith("fail-"))
            .map((id) => ({ id, reason: "succeeded" })),
          missing: [],
          failed: [],
        }), {
          status: 200,
    ...
    expect(await screen.findByText("失败任务一")).toBeInTheDocument()
    await user.click(screen.getByRole("button", { name: "选中本页已结束" }))
    expect(screen.getByText("已选 3 个")).toBeInTheDocument()
    expect(screen.getByRole("button", { name: "重试所选 (2)" })).toBeEnabled()

    await user.click(screen.getByRole("button", { name: "重试所选 (2)" }))
    expect(screen.getByText("确认重试所选失败任务？")).toBeInTheDocument()
    await user.click(screen.getByRole("button", { name: "确认重试" }))

    expect(await screen.findByText("已重试 2 个，跳过 1 个，不存在 0 个，失败 0 个。")).toBeInTheDocument()

    const resumeCall = mocks.fetch.mock.calls.find(
      ([input, init]) => String(input) === "/api/tasks/batch-resume" && init?.method === "POST",
    )
