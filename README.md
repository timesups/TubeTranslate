...
- 批量创建任务（链接 / 本地视频）
- 目录批处理：自动模式默认同时推进 3 条视频，GPU 推理、导入、音频合成和视频编码各限制为同时 1 个；翻译与 Azure TTS 请求各有全局 16 请求上限。手动模式仍按顺序逐阶段执行。
...
自动目录批处理的暂停会停止新条目和新阶段，正在执行的阶段或请求到达安全边界后退出；全部条目退出后才显示为已暂停。关闭「出错后继续」时，已经开始的视频允许完成，但不会再开始其他视频。删除正在处理的任务包后，其记录立即移除，会话文件在全部工作退出后清理，避免删除仍被使用的文件。
